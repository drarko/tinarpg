...
use ControllerPublic;

class TwitterController extends ControllerPublic
{
    protected $form;
    protected $storage;
    ...
    $me = $this->getServiceLocator()->get('ReverseOAuth2\Twitter');
    ...
    {
    $a = $this->getServiceLocator()->get('twitterConnect');
    ...
    if (strlen($this->params()->fromQuery('code')) > 10) {
		...
		if ($this->getAuthServiceStd()->hasIdentity()){
		      ...
		      $this->flashmessenger()->addMessage("Has asociado tu cuenta de Twitter al sitio.");
		      return $this->redirect()->toUrl('/'); 
	...
	$u = $this->getServiceLocator()->get('Usuarios/Model/Usuario');
        ...
        if ($request->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                return $this->redirect()->toUrl('/auth/twitter');
            }
